...
def install(env):
    """Install AZNFS via the distro feed, or the vendor installer on Azure Linux.

    Returns 0 on success, 3 on failure.
    """
    distribution = env.get("DISTRIBUTION", "")
    # Non-interactive install (no TTY prompts) for Packer builds.
    ...
    if "azurelinux" in distribution:
        ...
        try:
            ...
            return 3
        # The installer defaults to yum; Azure Linux uses tdnf.
        path = Path(script)
        path.write_text(path.read_text(encoding="utf-8").replace("yum", "tdnf"),
                        encoding="utf-8")
